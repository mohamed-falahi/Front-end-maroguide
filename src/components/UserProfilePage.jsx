...
export function UserProfilePage() {
    ...
    const fetchUserProfile = useCallback(async () => {
        ...
        if (userProfileCache.userId !== currentUserId) {
            console.log('User changed, resetting profile cache');
            userProfileCache.loaded = false;
            userProfileCache.userId = currentUserId;
            userProfileCache.profile = null;
            userProfileCache.posts = null;
            userProfileCache.stats = null;
        }

        // Check if cache is valid (within 2 minutes)
        const cacheAge = userProfileCache.lastFetch ? Date.now() - userProfileCache.lastFetch : Infinity;
        if (userProfileCache.loaded && userProfileCache.profile && cacheAge < 120000) {
            console.log('Using cached profile data');
            setProfileUser(userProfileCache.profile);
            setUserPosts(userProfileCache.posts || []);
            setStats(userProfileCache.stats || { posts: 0, followers: 0, following: 0 });
            setIsFollowing(userProfileCache.profile.is_following || false);
            setLoading(false);
            return;
        }
        ...
        try {
            ...
            if (userResponse && userResponse.success && userResponse.user) {
                ...
                userProfileCache.stats = statsData;
            }
            ...
            userProfileCache.posts = postsData;
            userProfileCache.loaded = true;
            userProfileCache.lastFetch = Date.now();
            userProfileCache.userId = currentUserId;
            ...
            if (profileResponse && profileResponse.success) {
                ...
                if (avatarFile) {
                    ...
                    if (avatarResponse && avatarResponse.success) {
                        avatarUploaded = true;
                        avatarPath = avatarResponse.avatar || avatarResponse.path || null;
                    } else {
                        showWarning("Avatar upload failed but continuing");
                    }
                }
                ...
                let coverPath = null;
                if (coverFile) {
                    const coverResponse = await uploadCover(coverFile);
                    if (coverResponse && coverResponse.success) {
                        coverUploaded = true;
                        coverPath = coverResponse.cover || coverResponse.path || null;
                    } else {
                        showWarning("Cover upload failed but continuing");
                    }
                }
                ...
                if (userFetched && updatedUser && updatedUser.id) {
                    if (setUser) {
                        setUser(updatedUser);
                    }
                    ...
                    if (avatarUploaded && coverUploaded) {
                        successMessage = "Profile, avatar, and cover updated successfully!";
                    } else if (avatarUploaded) {
                        successMessage = "Profile and avatar updated successfully!";
                    } else if (coverUploaded) {
                        successMessage = "Profile and cover updated successfully!";
                    }
                    ...
                    let successMessage = "Profile updated successfully!";
                    if (avatarUploaded && coverUploaded) {
                        successMessage = "Profile, avatar, and cover updated!";
                    } else if (avatarUploaded) {
                        successMessage = "Profile and avatar updated!";
                    } else if (coverUploaded) {
                        successMessage = "Profile and cover updated!";
                    }
                    showSuccess(successMessage);
                }
            } else {
                const errorMsg = profileResponse?.message || "Failed to update profile";
                showError(errorMsg);
            }
        } catch (error) {
    ...
    const GRID_POSTS = userPosts.length > 0
        ? userPosts.map(post => ({
            img: post.media && post.media.length > 0
                ...
                : "https://images.unsplash.com/photo-1548019979-2f0f98b6f42e?w=400&q=80",
